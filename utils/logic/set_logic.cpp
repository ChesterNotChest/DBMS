                                   ...
                                   const LogicRowContext &rowContext)
{
    if (node.children.size() != 3) {
        return {false, LogicTruthValue::Unknown, {QStringLiteral("BETWEEN node expects 3 children: lhs, lower, upper"), -1}};
    }
    ...
    const LogicCellValue lowerValue = cellValueFromNode(lowerNode);
    const LogicCellValue upperValue = cellValueFromNode(upperNode);

    const LogicTruthValue lowerCompare = compareValues(lhsValue, lowerValue, LogicCompareOperator::Gte);
    const LogicTruthValue upperCompare = compareValues(lhsValue, upperValue, LogicCompareOperator::Lte);

    if (lowerCompare == LogicTruthValue::False || upperCompare == LogicTruthValue::False) {
        return {true, node.negated ? LogicTruthValue::True : LogicTruthValue::False, {}};
    }

    if (lowerCompare == LogicTruthValue::Unknown || upperCompare == LogicTruthValue::Unknown) {
